...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${TMDB_API_BASE_URL}${TMDB_ENDPOINTS.TRENDING_MOVIES}/${timeWindow}?page=${page}`;
    ...
    const transformedResults = data.results.map((movie: IMovieSummaryDto) => ({
      ...movie,
      poster_path: movie.poster_path ? `${TMDB_IMAGE_URL}${TMDB_IMAGE_QUALITY.POSTER_SIZES.W_185}${movie.poster_path}` : null,
      backdrop_path: movie.backdrop_path ? `${TMDB_IMAGE_URL}${TMDB_IMAGE_QUALITY.BACKDROP_SIZES.W_780}${movie.backdrop_path}` : null,
    }));
